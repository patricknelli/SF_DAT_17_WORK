...
distances = [(euclidean_distance(unknown, iris_data[i,0:4]),iris_data[i,-1]) for i in range(len(iris_data))] # <FILL IN>
...
nearest = sorted(distances, key=lambda distance: distance[0])[0:k] # <FILL IN>
...
prediction = Counter([nearest[i][1] for i in range(len(nearest))]).most_common(1)[0][0] # <FILL IN>
...
def predict(unknown, k = 3):
    ...
    '''
    distances = [(euclidean_distance(unknown, iris_data[i,0:4]),iris_data[i,-1]) for i in range(len(iris_data))]
    nearest = sorted(distances, key=lambda distance: distance[0])[0:k]
    prediction = Counter([nearest[i][1] for i in range(len(nearest))]).most_common(1)[0][0]
    return prediction 
