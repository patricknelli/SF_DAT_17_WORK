...
plt.scatter(drinks['spirit_servings'], drinks['wine_servings'])
plt.show();

# 7. Show a list of the top 10 spirit drinking countries 
# (show only country names and spirit servings)
drinks[['country', 'spirit_servings']].sort('spirit_servings', ascending=False).head(10)
...
drinks['wine_servings'].groupby(drinks['continent']).mean().idxmax()
#looks like it is the EU
